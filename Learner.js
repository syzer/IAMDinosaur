...
Learn.executeGeneration = function () {
  ...
  async.mapSeries(Learn.genomes, Learn.executeGenome, argument => {
    // Kill worst genomes
    Learn.genomes = Learn.selectBestGenomes(Learn.selection)

    // Copy best genomes
    const bestGenomes = _.clone(Learn.genomes)
...
Learn.selectBestGenomes = function (selectN) {
  const selected = _.sortBy(Learn.genomes, 'fitness').reverse()
...
Learn.buildGenome = function (inputs, outputs) {
  ...
  const network = new Architect.Perceptron(inputs, 4, 4, outputs)
