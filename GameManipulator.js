...
const COLOR_DINOSAUR = '535353'
const DARK_COLOR_DINO = 'ACACAC'
...
GameManipulator.computePoints = function () {
  for (const k in GameManipulator.sensors) {
    const sensor = GameManipulator.sensors[k]
...
GameManipulator.readSensors = function () {
  const offset = GameManipulator.offset
  ...
  for (var k in GameManipulator.sensors) {
    ...
    const forward = sensor.value * GameManipulator.width * 0.8 * sensor.length

    const end = Scanner.scanUntil(
      // Console.log(
        // Start position
        [start[0], start[1]],
        // Skip pixels
    ...
    if (end) {
      ...
      let endPoint = Scanner.scanUntil(
        [end[0] + 75, end[1]],
        [-2, 0],
        COLOR_DINOSAUR,
        false,
      ...
      const sizeTmp = (endPoint[0] - end[0]) / 100.0
    ...
    // Compute speed
    const dt = (Date.now() - sensor.lastComputeSpeed) / 1000
    sensor.lastComputeSpeed = Date.now()

    if (sensor.value < sensor.lastValue) {
      // Compute speed
      const newSpeed = (sensor.lastValue - sensor.value) / dt
